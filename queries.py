...
def display_categories(sess):
    ...
    """
    Categorys = sess.query(Category).all()
...
def display_products(sess,cp):
    ...
    """
    result = sess.query(Product.product_name,Product.bar_code, Product.url,Product.nutrition_grade,Product.energy,Product.proteins).distinct(Product.product_name,Product.bar_code,Product.url,Product.nutrition_grade,Product.energy,Product.proteins).filter(
...
def display_better(sess,product_id,cid):
    ...
    for grade in current_product:
        ...
        if grade.nutrition_grade == 1:
            ...
            recomment_product = sess.query(Product.product_name,Product.bar_code,Product.url,Product.nutrition_grade,Product.energy,Product.proteins,Store.store_name).filter(
...
# select_brands(db)
# get_product(db,1)
# select_stores(db,'Steak Haché 5%')
# display_better(db,'3245411954277',cp=1) #0
# display_better(db,'3181232120043',cp=1) # 1

# display_better(db,'Steak Haché Pur Bœuf 15%')
